...
from app.providers.base import MarketDataProvider, ProviderDataError, SpotQuote
...
class RuntimeState:
    def __init__(
        ...
        if self.repository is None and self.settings.database_url:
            self.repository = PostgresRepository.connect(self.settings.database_url)
    ...
    def list_tickers(self) -> list[AppTicker]:
        ...
        return sorted((ticker for ticker in self.tickers.values() if ticker.enabled), key=lambda item: item.ticker)
    ...
    def analyze_latest(
        ...
        *,
        ...
        if not self.repository and idempotency_key is not None:
    ...
    def _latest_capture_record(self, ticker: str) -> tuple[int | None, ChainCapture | None]:
        ...
        captures = self.chain_captures.get(ticker, [])
